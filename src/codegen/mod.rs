...
pub fn generate_all(manifest: &Manifest, output_dir: &str) -> Result<()> {
    ...
    println!("  [stub] Phronesis codegen for '{}' — implementation pending", manifest.workload.name);
    Ok(())
}

pub fn build(manifest: &Manifest, _release: bool) -> Result<()> {
    println!("Building phronesiser workload: {}", manifest.workload.name);
    Ok(())
}

pub fn run(manifest: &Manifest, _args: &[String]) -> Result<()> {
    println!("Running phronesiser workload: {}", manifest.workload.name);
    Ok(())
}
