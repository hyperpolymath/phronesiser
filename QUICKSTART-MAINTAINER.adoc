...
// Replace phronesiser, phronesiser, {{DEPS}} with actuals
...
| `$PREFIX/share/phronesiser/`
| Data files, assets

| `$PREFIX/share/doc/phronesiser/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/phronesiser/config.toml`

Fallback: `$HOME/.config/phronesiser/config.toml`
...
Or via OPSM: `opsm update phronesiser`
...
[source,bash]
----
just install --prefix=/opt/phronesiser-instance1 --config=/etc/phronesiser/instance1.toml
just install --prefix=/opt/phronesiser-instance2 --config=/etc/phronesiser/instance2.toml
----
